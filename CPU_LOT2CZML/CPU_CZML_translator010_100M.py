...
# 経度の調整
df.loc[df['PSI_flag'] == 's', 'longitude'] += 0.001
df.loc[df['PSI_flag'] == 'p', 'longitude'] -= 0.001
...
# BOXの色を設定と透明度の変更
color_dict = {'s': [0, 0, 255, 100], 'i1': [165, 42, 42, 100], 'p': [255, 255, 0, 100]}
...
for _, row in df.iterrows():
    czml.append({
        ...
        "position": {
            ...
            "cartographicDegrees": [row['longitude'], row['latitude'], row['height']*lot_length ]
        ...
        "box": {
            "dimensions": {
                ...
                # BOXの一辺の長さを計算は初期処理で済み。lot_sizeを1mにする
                "cartesian": [row['lot_size'], row['lot_size'], row['lot_size']]
